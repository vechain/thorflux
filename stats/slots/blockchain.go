...
func parseAuthorityNodeResponse(data string) (types.AuthorityNodeList, error) {
	if len(data) < 2 {
	...
	if len(data) < 128 {
		...
		return types.AuthorityNodeList{}, nil
	}
	...
	nodes := make(types.AuthorityNodeList, amount.Uint64())
	for index := uint64(0); index < amount.Uint64(); index++ {
		...
		nodes[index] = types.AuthorityNode{
			Master:   master,
			Endorsor: endorsor,
			Active:   active,
		}
